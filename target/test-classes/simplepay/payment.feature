Feature: Payment
  ...
  Scenario: Customer has insufficient funds
    Given a customer with id "cid1"
    And a merchant with id "mid1"
    When the merchant initiates a payment for 1000 kr by the customer
    Then the payment is not successful
    And an error message is returned saying "customer with id=cid1 has insufficient funds to pay 1000dkk"
